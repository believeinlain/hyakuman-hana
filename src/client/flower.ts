...
export class Flower {
    ...
    static spawnFromGenome(genome: FlowerGenome, position: Vector3, scene: Scene) {
        let newPacket = new FlowerPacket(
            uuidv4(), 
            {x:position.x, y:position.z}, 
            genome);
        return new Flower(newPacket, scene);
    }

    constructor(info: FlowerPacket, scene: Scene) {
        this.info = info;

        let position = new Vector3(this.info.location.x, 25, this.info.location.y);
